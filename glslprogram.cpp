...
GLSLProgram::CreateHelper(char* file0, ...)
{
	...
	while (file != NULL)
	{
		...
		if (!SkipToNextVararg)
		{
			...
			auto err = fopen_s(&stream, file, "rb");
			if (err != NULL)
			{
			...
			if (!SkipToNextVararg)
			{
				...
				fread(buf, sizeof(GLchar), length, stream);
				buf[length] = '\0';
				fclose(stream);
				...
				if (compileStatus == 0)
				{
					...
					if (infoLogLen > 0)
					{
						...
						auto log_err = fopen_s(&logfile, "glsllog.txt", "w");
						if (log_err == NULL)
						{
							fprintf(logfile, "\n%s\n", infoLog);
							fclose(logfile);
...
#define CHECK_GL_ERRORS
		...
		GLSLProgram::SaveProgramBinary(const char* fileName, GLenum * format)
	{
		...
		FILE** stream = nullptr;
		auto err = fopen_s(stream, fileName, "wb");
		if (err == NULL)
		{
	...
	{
		FILE** stream = nullptr;
		auto err = fopen_s(stream, fileName, "rb");
		if (err == NULL)
		{
		...
		fseek(*stream, 0, SEEK_END);
		GLint length = (GLint)ftell(*stream);
		GLubyte* buffer = new GLubyte[length];
		rewind(*stream);
